...
export function generateHairLines0(faceCountour, numHairLines = 100) {
  ...
  for (var i = 0; i < numHairLines; i++) {
    ...
    for (var j = 0; j < numHairPoints; j++) {
      d.push([
        d0[j][0] * (j * (1 / numHairPoints)) ** 2 +
          d1[j][0] * (1 - (j * (1 / numHairPoints)) ** 2),
        d0[j][1] * (j * (1 / numHairPoints)) ** 2 +
          d1[j][1] * (1 - (j * (1 / numHairPoints)) ** 2),
      ]);
    }
...
export function generateHairLines2(faceCountour, numHairLines = 100) {
  ...
  for (var i = 0; i < numHairLines; i++) {
    ...
    var lastHairPoint =
      results[results.length - 1][results[results.length - 1].length - 1];
    var lastPointsDistance = Math.sqrt(
