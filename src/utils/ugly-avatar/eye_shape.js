...
function generateEyeParameters(width) {
  ...
  let offset_upper_left_x = randomFromInterval(
    -eye_true_width / 10.0,
    eye_true_width / 2.3
  ); // Upper eyelid control point offset to create asymmetry
  let offset_upper_right_x = randomFromInterval(
    -eye_true_width / 10.0,
    eye_true_width / 2.3
  ); // Upper eyelid control point offset to create asymmetry
  ...
  let offset_lower_left_x = randomFromInterval(
    offset_upper_left_x,
    eye_true_width / 2.1
  ); // Lower eyelid control point offset
  let offset_lower_right_x = randomFromInterval(
    offset_upper_right_x,
    eye_true_width / 2.1
  ); // Upper eyelid control point offset to create asymmetry
  let offset_lower_left_y = randomFromInterval(
    -offset_upper_left_y + 5,
    height_lower
  ); // Upper eyelid control point offset to create asymmetry
  let offset_lower_right_y = randomFromInterval(
...
export function generateEyePoints(rands, width = 50) {
  ...
  let upper_eyelid_left_control_point = [
  ...
  let upper_eyelid_right_control_point = [
  ...
  let lower_eyelid_left_control_point = [
  ...
  let lower_eyelid_right_control_point = [
...
export function generateBothEyes(width = 50) {
  ...
  for (let key in rands_right) {
    ...
    if (typeof rands_right[key] === "number") {
      ...
      rands_right[key] += randomFromInterval(
        -rands_right[key] / 2.0,
        rands_right[key] / 2.0
      );
    }
